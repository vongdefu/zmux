...
} from "../services/recommendedPlaylists"
  ...
  if (state.playContext.type === "playlist") {
    return (
      state.playlists.find(
        (playlist) => playlist.id === state.playContext.playlistId,
      )?.tracks || []
    )
  }
  if (state.playContext.type === "recommended") {
    return (
      state.recommendedPlaylists.find(
        (playlist) => playlist.id === state.playContext.playlistId,
      )?.tracks || []
    )
  }
  return orderedResults.value.length
...
function interleaveTracks(list) {
  ...
  Object.values(grouped).forEach((tracks) =>
    tracks.sort((a, b) => (a.displayIndex || 0) - (b.displayIndex || 0)),
  )
...
async function search(reset = true) {
  ...
  state.statusText = state.searchResults.length
    ? `找到 ${state.searchResults.length} 首`
    : "暂无结果"
...
function loadMore() {
  ...
  enabled.forEach((source) => {
    if (source === "netease") {
      ...
      state.perSourceCurrentLimit[source] =
        (state.perSourceCurrentLimit[source] || state.perSourceLimit) +
...
async function playFromList(type, index, playlistId = null) {
  ...
  if (type === "favorites") list = state.favorites
    ...
    list =
      state.playlists.find((playlist) => playlist.id === playlistId)?.tracks ||
      []
  else if (type === "recommended")
    list =
      state.recommendedPlaylists.find((playlist) => playlist.id === playlistId)
        ?.tracks || []
  else
    list = orderedResults.value.length
      ? orderedResults.value
      : state.searchResults
...
function isFavorite(track) {
  return Boolean(
    track && state.favorites.some((item) => item.uid === track.uid),
  )
}
...
function importLibrary(data) {
  ...
  const mergeTrack = (target, rawTracks) => {
    ...
    rawTracks.forEach((raw) => {
      ...
      if (!imported?.uid || target.some((item) => item.uid === imported.uid))
        return
  ...
  if (Array.isArray(data.playlists)) {
    data.playlists.forEach((rawPlaylist, index) => {
      ...
      let playlist =
        state.playlists.find((item) => item.id === rawPlaylist.id) ||
        state.playlists.find((item) => item.name === name)
      if (!playlist) {
        playlist = {
...
function rebuildTrackMap() {
  ;[
    ...
    ...state.recommendedPlaylists.flatMap((playlist) => playlist.tracks || []),
  ].forEach((track) => {
...
async function fetchRecommendedPlaylistTracks(playlistId) {
  const playlist = state.recommendedPlaylists.find(
