...
export async function searchJoox(keyword, limit = 10) {
  ...
  const json = await response.json()
  const songs =
    json?.code === 200 && Array.isArray(json?.data?.songs)
      ? json.data.songs
      : []
...
export async function searchSource(source, keyword, options) {
  if (source === "netease")
    return searchNetease(keyword, options.page, options.limit)
...
async function fetchQQDetails(track) {
  ...
  const response = await fetch(url)
  const data = await response.json()
  if (!data || typeof data !== "object" || !data.song_mid)
    throw new Error("QQ detail response invalid")
  ...
  track.qqQualityText =
    best.text || (data.vip ? `VIP:${data.vip}` : null) || track.qqQualityText
...
function pickQQPlayUrl(data) {
  const candidates = [
    [
      ...
      "lossless",
      "LOSSLESS",
      `SQ ${data.kbps_sq || ""}`.trim(),
    ],
    [
      "song_play_url_pq",
      "lossless",
      "LOSSLESS",
      `PQ ${data.kbps_pq || ""}`.trim(),
    ],
    [
      "song_play_url_accom",
      "hq",
      "HQ",
      `ACCOM ${data.kbps_accom || ""}`.trim(),
    ],
  ...
  return found
    ? { url: data[found[0]], tag: found[1], label: found[2], text: found[3] }
    : { url: null, tag: null, label: null, text: null }
}
...
async function fetchKuwoDetails(track) {
  ...
  if (!json || json.code !== 200 || !json.data)
    throw new Error("Kuwo detail response invalid")
...
async function fetchJooxDetails(track) {
  ...
  const json = await response.json()
  if (!json || json.code !== 200 || !json.data)
    throw new Error("JOOX detail response invalid")
...
function pickJooxPlayUrl(links) {
  ...
  for (const name of order) {
    ...
    if (match)
      return { url, tag: `${match[1]}k`, label: `${match[1]}K`, text: name }
